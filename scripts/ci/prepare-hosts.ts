...
import { json, output, readInput, record, root, run } from './common.ts';
...
function copyExample(example: string, host: string) {
...
for (const [name, host] of Object.entries(hosts)) {
  ...
  const manifest = json<import('../validation-types.ts').PackageManifest>(path.join(host, 'package.json'));
  ...
  if (name === 'EXPO_HOST') {
    ...
    const config = json<{ expo: { name: string; slug: string; ios: { bundleIdentifier: string }; android: { package: string } } }>(path.join(host, 'app.json'));
  ...
  } else run(`${name}-prepare`, process.execPath, [path.join(root, 'packages', sdk, 'test/native-artifacts/prepare.ts'), host, ...artifacts], root, env);
