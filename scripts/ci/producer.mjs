...
import { commit, digest, json, output, record, root, run } from './common.mjs';
...
for (const sdk of ['cli', 'react-native', 'lynx']) {
  ...
  const { name, version } = json(path.join(root, 'packages', sdk, 'package.json'));
...
for (const [label, file, extra] of [
  ['relocation-ios', 'native-export/ios.mjs', {}],
  ['relocation-android-build', 'native-export/android.mjs', { ANDROID_CONSUMER_BUILD_ONLY: '1' }],
  ['async-export', 'native-export/async.mjs', {}],
