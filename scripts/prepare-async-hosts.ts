...
assert(rn && lynx, 'Usage: node scripts/prepare-async-hosts.ts RN_BASELINE_HOST LYNX_BASELINE_HOST');
const report = JSON.parse(readFileSync(path.join(root, 'target/async-protocol/export-report.json'), 'utf8'));
assert(report.desktopAsyncParity && report.producerDeleted, 'Complete the async export gate first');

function edit(file: string, update: (source: string) => string) {
...
for (const [host, sdk] of [[rn, 'react-native'], [lynx, 'lynx']] as const) {
