import type { BridgeMessage, BridgeEvent, EventPayload, TauriInternals } from './webview-types.ts';
...
(() => {
  ...
  const eventCallbacks = new Map<number, { callback(event: BridgeEvent): void; once: boolean }>();
  const eventListeners = new Map<number, { event: string; handler: number }>();
  let nextCallback = 1;
  let nextListener = 1;
  let readySent = false;
  const post = (message: BridgeMessage) => __TAURI_NATIVE_POST_MESSAGE__;
  const send = (message: BridgeMessage) => post({ ...message, document: documentId });
  ...
  function finish(id: string, responseJson?: string, cause?: unknown) {
  ...
  function failAll(cause: unknown) {
  ...
  const unlisten = (event: string, id: number) => {
  ...
  const validEvent: (event: unknown) => asserts event is string = (event) => {
  ...
  const validTarget = (target: EventPayload['target']) => {
  ...
  function invokeEvent(command: string, payload: EventPayload) {
    try {
      if (!active) throw error('closed_document');
