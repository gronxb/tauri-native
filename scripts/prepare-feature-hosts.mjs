...
assert(rn && lynx, 'Usage: node scripts/prepare-feature-hosts.mjs RN_HOST LYNX_HOST [EXPO_HOST]');
...
const report = JSON.parse(readFileSync(path.join(evidence, 'export-report.json')));
assert(report.desktopFrontend.every(result => result.passed) && report.producerDeleted, 'Complete the document export gate first');
for (const [host, sdk, bare] of [[rn, 'react-native', true], [lynx, 'lynx', false], ...(expo ? [[expo, 'react-native', false]] : [])]) {
