...
export interface RetainedAndroidArtifact {
  formatVersion: 2;
  abiVersion: 3;
  platform: 'android';
...
export function readRetainedArtifacts(directory: string): RetainedAndroidArtifact {
  ...
  try {
    ...
    const manifest = JSON.parse(readFileSync(path.join(directory, 'manifest.json'), 'utf8')) as RetainedAndroidArtifact;
    if (manifest?.formatVersion !== 2 || manifest.abiVersion !== 3) fail('artifact_format', 'requires format 2 / retained ABI 3');
    if (manifest.platform !== 'android') fail('artifact_platform', 'this retained integration supports Android');
    ...
    if (manifest.bootstrap?.owner !== 'tauri' || manifest.bootstrap.project !== 'android' || manifest.bootstrap.minimumApiLevel !== 24 ||
    ...
    if (!Array.isArray(manifest.native) || !manifest.native.length || new Set(manifest.native.map(slice => slice.abi)).size !== manifest.native.length ||
    ...
    const files = new Map<string, RetainedAndroidArtifact['files'][number]>();
    ...
    visit();
    const required = ['commands.json', 'callers.json', 'include/tauri_native_runtime.h', 'android/settings.gradle', 'android/tauri.settings.gradle',
      'android/gradlew', 'android/app/build.gradle.kts', 'android/app/src/main/AndroidManifest.xml',
      'android/app/src/main/java/dev/taurinative/runtime/RuntimeSession.java', ...manifest.native.map(slice => slice.path)];
