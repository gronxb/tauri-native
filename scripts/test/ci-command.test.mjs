...
import { buildAndInstallTestHost } from '../../packages/cli/test/native-export/host-build.mjs';
...
test('a failed Android installation retains its error when diagnostic collection also fails', () => {
  ...
  try {
    ...
    assert.throws(() => buildAndInstallTestHost({ name: 'rn', directory }, 'android', (label, command, args) => {
