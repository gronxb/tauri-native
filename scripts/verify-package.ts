...
) as import('./validation-types.ts').PackageManifest;
...
const packed = (JSON.parse(match[2]!) as { name: string; version: string; id: string; entryCount: number; unpackedSize: number; files: { path: string; mode: number }[] }[])[0]!;
