...
interface SavedDocument { title: string; body: string }
...
async function search() {
  ...
  try {
    const documents = await invoke<SavedDocument[]>('search_documents', { directory: await directory, query: query.value });
...
document.querySelector<HTMLFormElement>('#editor')!.onsubmit = async event => {
  event.preventDefault();
  const save = document.querySelector<HTMLButtonElement>('#save')!;
  save.disabled = true;
  const version = ++searchVersion;
  try {
    const saved = await invoke<SavedDocument>('save_document', { directory: await directory, title: title.value, body: body.value });
...
document.querySelector<HTMLFormElement>('#search-form')!.onsubmit = event => { event.preventDefault(); search(); };
