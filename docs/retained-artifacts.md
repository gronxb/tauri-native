...
Native libraries remap Rust producer, generated integration, target and Cargo-home paths and strip native debug records before publication. Explicit environment compiler flags survive, including encoded flags and paths with spaces. Each native library is scanned for those original paths and still must pass the architecture/startup/plugin/ABI checks. Compiler scenarios now exercise ordinary Cargo configuration, encoded environment flags, plain RUSTFLAGS and an existing compiler wrapper. Cargo resolves these flags before the generated wrapper adds remapping; it does not replace configured rustflags. A compiler wrapper declared only in Cargo configuration requires an explicit RUSTC_WRAPPER or CARGO_BUILD_RUSTC_WRAPPER selection, so it cannot be silently discarded.
...
Retained exports preserve an existing ordinary iOS Xcode project and Android Gradle project instead of running init again. In Tauri CLI 2.11.4, iOS init regenerates the Xcode project/Info.plist, and Android init rewrites its Rust build task. Initial scaffolding still uses the ordinary Tauri CLI when no native project exists. `portable.ts <platform> --release --native-config` adds ordinary native settings before export, hashes those native inputs, and checks the configuration/resources in the compiled app after producer deletion.

The [iOS configuration gate](evidence/retained-tauri-native-config-ios-2026-09-09.json) reproduces a pre-fix iOS deployment target regression from 15.0 to 14.0, then verifies that 15.0 and a producer-authored Info.plist key survive alongside the complete native plugin/lifecycle scenarios. This is scoped native configuration evidence, not support for arbitrary native build scripts or external Cargo configuration hierarchies.

Compiler intermediates use an export-owned temporary child of CARGO_TARGET_DIR (removed on success/failure). This prevents reuse of libraries compiled without the wrapper's remapping/link flags; unchanged `--incremental` artifacts still avoid native compilation. The [Cargo compiler-wrapper environment](https://doc.rust-lang.org/cargo/reference/config.html#buildrustc-wrapper) is forwarded through CARGO_* names because [Tauri CLI's mobile environment filter](https://github.com/tauri-apps/tauri/blob/tauri-cli-v2.11.4/crates/tauri-cli/src/mobile/mod.rs#L306) does not match RUSTC_WRAPPER or RUSTFLAGS. Explicit environment flags and workspace wrappers are carried across the same boundary. Path/symbol/architecture checks still run before publication.


The matching [Android configuration gate](evidence/retained-tauri-native-config-android-2026-09-09.json) also passes: the authored Manifest metadata and asset remain in the compiled Release/R8 APK, followed by the complete native permissions/callback-retirement/deep-link/persistence gate and 16 KB checks. Both platforms verify unchanged native input bytes across successful export and invalid-capability failure, retain the previous complete receipt on failure, and consume the relocated artifact after deleting the producer. CLI scenario tests (65), typechecks and CLI package contents pass. All-slice link evidence above was recorded before this compiler-wrapper refinement; these configuration runs execute the refined exporter on arm64.


ABI 3 runtimes advertising the `events` feature now expose native listen/poll/unlisten through the packaged Android/iOS session clients. They use the original Tauri event handler and check both declared caller grants and the live Tauri authority/document context. The source-free arm64 Release gates verify original save/deep-link events, cancellation, native handler cleanup and exactly-once delivery after remount: [iOS](evidence/retained-tauri-events-ios-2026-09-09.json), [Android](evidence/retained-tauri-events-android-2026-09-09.json). [ADR 0008](adr/0008-retained-native-caller-boundary.md#native-event-subscriptions) specifies bounds and overflow recovery. Native channels and RN/Expo/Lynx package-level event scheduling/lifecycle remain open.


The initial Android page can execute IPC before Wry's `onPageStarted` callback. Volatile visibility alone does not fix that ordering: [the pre-fix probe failed twice in ten starts](evidence/retained-tauri-android-origin-race-2026-09-09.json). Capture now records the actual initial main-frame URL when Tauri provides its protocol response, before document JavaScript can run. Subresources and unhandled requests cannot populate this initial context; no fixed local URL or ACL grant is inserted. [Ten Release/R8 cold starts pass](evidence/retained-tauri-android-origin-recovery-2026-09-09.json), including an artificial two-second delay of the later page-start cache write. The test uses the production capture function and identical Rust libraries; full renderer integration remains separately gated.
