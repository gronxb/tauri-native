...
For the existing calculator regression, replace both received platform directories with complete legacy calculator exports and restore the example's `src/App.tsx`. Rebuild the bundle, run `pod install` again to refresh changed archive names, and rebuild/install both Release hosts. Copy each example `.maestro/*-integration.yaml` to a temporary flow with its `appId` changed to the unique QA identifier, then run it on the matching platform. Do not change the producer frontend or command implementation.
