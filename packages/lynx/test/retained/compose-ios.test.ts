...
const { composeIos } = createRequire(import.meta.url)('../../compose.cjs') as { composeIos: (options: IosCompositionOptions) => { changed: boolean; minimumOsVersion: string } };
const roots: string[] = [];
...
function fixture() {
  ...
  receipt();
  write(renderer, 'package.json', '{"private":true}'); write(renderer, 'main.lynx.bundle', 'first renderer');
  const options = { artifactsDir: artifact, outputDir: path.join(root, 'generated app'), bundleFile: path.join(renderer, 'main.lynx.bundle') };
...
test('iOS composition preserves its source and settings, keeps the original minimum OS and regenerates without losing consumer files', () => {
  const f = fixture(), before = snapshot(f.artifact);
  assert.equal(f.run().minimumOsVersion, '15.0'); const first = snapshot(f.options.outputDir);
  ...
  assert.equal(project.objects.release.buildSettings.IPHONEOS_DEPLOYMENT_TARGET, '15.0');
  ...
  write(f.options.outputDir, 'ios/consumer.txt', 'keep'); write(f.renderer, 'main.lynx.bundle', 'upgrade');
  assert.equal(f.run().changed, true);
  assert.equal(readFileSync(path.join(f.options.outputDir, 'ios/consumer.txt'), 'utf8'), 'keep');
  assert.equal(readFileSync(path.join(f.options.outputDir, 'ios/assets/tauri-native-lynx/main.lynx.bundle'), 'utf8'), 'upgrade');
});

test('a higher authored deployment target remains higher than the Lynx minimum', () => {
...
test('CocoaPods receipt tracking accepts its project rewrite but rejects existing or concurrent source edits', () => {
  ...
  let result = run('TauriNativeLynxRetained.composition_receipt(ARGV[0])');
  ...
  result = run(`
    before = TauriNativeLynxRetained.composition_receipt(ARGV[0])
    File.write(File.join(ARGV[0], 'Sources/App/main.mm'), 'concurrent consumer edit')
    TauriNativeLynxRetained.finish_composition(ARGV[0], before)
  `);
  assert.notEqual(result.status, 0); assert.match(result.stderr, /composition file changed/);
  assert.equal(readFileSync(receipt, 'utf8'), before);
  ...
  result = run(`
    ...
    TauriNativeLynxRetained.finish_composition(ARGV[0], before)
    TauriNativeLynxRetained.composition_receipt(ARGV[0])
  `);
