...
/** A generated TurboModule calling the original Tauri application on main. */
@ReactModule(name = NativeTauriRuntimeSpec.NAME)
public final class TauriRuntimeModule extends NativeTauriRuntimeSpec {
  private final Scope scope = new Scope();

  public TauriRuntimeModule(ReactApplicationContext context) { super(context); }
  void retire() { scope.close(); }

  @Override public void exchange(String operation, Callback callback) {
    scope.main.post(() -> {
  ...
  @Override public void invalidate() { scope.main.post(scope::close); super.invalidate(); }
  ...
  /** Owned by one RN engine, never shared across engine replacements. Main thread only. */
  public static final class Scope implements AutoCloseable {
    ...
    @Override public void close() {
      if (Looper.myLooper() != Looper.getMainLooper()) throw new IllegalStateException("Close the RN scope on main");
      if (closed) return;
      closed = true;
