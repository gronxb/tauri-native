...
function readRetainedArtifacts(directory) {
	...
	try {
		if (!(0, node_fs.lstatSync)(directory).isDirectory() || !(0, node_fs.lstatSync)(node_path.default.join(directory, "manifest.json")).isFile()) fail("artifact_symlink", "root and manifest must be regular paths");
		...
		function visit(prefix = "") {
			for (const entry of (0, node_fs.readdirSync)(node_path.default.join(directory, prefix), { withFileTypes: true })) {
				...
				if (entry.isDirectory()) visit(relative);
					...
					const bytes = (0, node_fs.readFileSync)(node_path.default.join(directory, relative)), receipt = files.get(relative);
		...
		const commands = JSON.parse((0, node_fs.readFileSync)(node_path.default.join(directory, manifest.commands), "utf8"));
		...
		const policy = JSON.parse((0, node_fs.readFileSync)(node_path.default.join(directory, manifest.callers), "utf8"));
		...
		if ((0, node_crypto.createHash)("sha256").update((0, node_fs.readFileSync)(node_path.default.join(directory, "callers.json"))).digest("hex") !== manifest.source.callerPolicySha256) fail("artifact_policy", "caller policy differs from compiled policy receipt");
		if (manifest.source.buildSha256 || manifest.source.buildReceiptSha256) {
			const bytes = (0, node_fs.readFileSync)(node_path.default.join(directory, "build.json"));
...
//#endregion
exports.readRetainedArtifacts = readRetainedArtifacts;
exports.retainedAndroidAbis = retainedAndroidAbis;
