...
import { commandOutput, nativeTool } from '../discovery/native-tool.ts';
...
export interface RetainedAndroidOptions {
  tauriDir: string;
  outputDir?: string;
  callerPolicy?: string;
  targets?: string;
...
export function exportRetainedAndroid(options: RetainedAndroidOptions): void {
  if (options.manifest || options.watch || options.incremental || options.force) throw new Error('Retained export currently requires a full ordinary-producer build; legacy manifests, incremental cache and watch need retained-runtime acceptance.');
  ...
  const output = path.resolve(options.outputDir ?? path.join(project.tauriDirectory, 'gen/tauri-native/retained-android'));
  ...
  if (typeof applicationId !== 'string' || !/^[a-zA-Z_]\w*(?:\.[a-zA-Z_]\w*)+$/.test(applicationId)) throw new Error('Retained Android export requires a Java-compatible Tauri identifier.');
  ...
  for (const [name, version] of [['wry', '0.55.1'], ['tauri-runtime-wry', '2.11.4']]) {
    ...
    if (found.length !== 1 || found[0]!.version !== version) throw new Error(`Retained Android native integration verifies ${name}@${version}.`);
  }
  ...
  // Tauri CLI uses an app-identity options file even across platforms/output paths.
  const bootstrapLock = path.join(tmpdir(), `tauri-native-mobile-${sha256(applicationId).slice(0, 24)}.lock`);
  try { writeFileSync(bootstrapLock, `${process.pid}\n`, { flag: 'wx' }); }
  catch (error) {
    if ((error as NodeJS.ErrnoException).code !== 'EEXIST') throw error;
    throw new Error(`Another retained mobile export owns ${bootstrapLock}. Remove it only after its recorded process has stopped.`);
  }
  ...
  try {
    ...
    publishArtifacts(output, stage => {
      ...
      // Tauri's generated Gradle task reconstructs the package-manager command.
      // An absolute `node tauri.js` invocation becomes an invalid `node tauri`.
      const npmManifest = path.join(cwd, 'package.json');
      const npmPackage = JSON.parse(readFileSync(npmManifest, 'utf8'));
      npmPackage.scripts = { ...npmPackage.scripts, 'tauri-native:runtime': 'tauri' };
      writeFileSync(npmManifest, JSON.stringify(npmPackage, null, 2) + '\n');
  ...
  } finally { try { runtime?.cleanup(); } finally { rmSync(bootstrapLock, { force: true }); } }
}
