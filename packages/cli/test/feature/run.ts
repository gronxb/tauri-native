...
import { hostProfiles, buildAndInstallTestHost } from '../native-export/host-build.ts';

import type { FeatureExport, MaestroCommand } from '../../../../scripts/validation-types.ts';
...
for (const tool of ['cargo', 'rustc']) assert.equal((spawnSync(tool, ['--version'], { env: hostEnv }).error as NodeJS.ErrnoException | undefined)?.code, 'ENOENT', `${tool} must be absent during host builds`);
const profiles: import('../native-export/host-build.ts').InstalledHost[] = hostProfiles.map(profile => ({ ...profile, directory: process.env[profile.hostVariable]! }));
...
function run(label: string, command: string, args: string[], cwd = root, env: NodeJS.ProcessEnv = process.env) {
...
if (!process.env.FIELDNOTES_PACKAGES) run('export', process.execPath, ['--experimental-strip-types', path.join(root, 'packages/cli/test/feature/export.ts')]);
const exported = JSON.parse(readFileSync(path.join(evidence, 'export-report.json'), 'utf8')) as FeatureExport;
...
const packages = {} as Record<'react-native' | 'lynx', string>;
for (const sdk of ['react-native', 'lynx'] as const) {
...
run('prepare-hosts', process.execPath, [path.join(root, 'scripts/prepare-feature-hosts.ts'), ...profiles.map(profile => profile.directory)], root, hostEnv);
...
writeFileSync(pendingFlow, readFileSync(path.join(root, 'scripts/feature-pending-contract.yaml'), 'utf8')
  .replace('appId: ${APP_ID}', 'appId: ${APP_ID}\nandroidWebViewHierarchy: devtools'));
if (changedRust) {
  const baseline = JSON.parse(readFileSync(path.join(root, 'target/document-feature/export-report.json'), 'utf8')) as FeatureExport;
...
for (const profile of profiles) {
  ...
  for (const platform of platforms) {
    ...
    try {
      ...
      assert.deepEqual(inventory(received), exported.artifacts[platform as 'ios' | 'android'].files, 'Only copied artifacts are consumed');
      ...
      const commands = (JSON.parse(readFileSync(path.join(debug, 'commands-(feature-contract).json'), 'utf8')) as MaestroCommand[])
      ...
      if (changedRust) {
        ...
        const pendingCommands = (JSON.parse(readFileSync(path.join(pendingDebug, 'commands-(feature-pending-contract).json'), 'utf8')) as MaestroCommand[])
          .sort((left, right) => left.metadata.sequenceNumber - right.metadata.sequenceNumber);
      ...
      results.push({ host: profile.name, platform, appId: profile.appId, passed: false, error: error instanceof Error ? error.message : String(error) });
      console.error(`FAIL: ${label}; continuing the other independent consumers. ${error instanceof Error ? error.message : String(error)}`);
      if (platform === 'ios') {
        try {
        ...
        } catch (diagnosticError) { console.error(`Could not collect ${label} diagnostics: ${diagnosticError instanceof Error ? diagnosticError.message : String(diagnosticError)}`); }
      }
    }
  }
}
