...
function run(command: string, args: string[], options: Omit<import('node:child_process').SpawnSyncOptions, 'encoding'> & { measure?: string } = {}) {
...
let desktop: import('node:child_process').ChildProcess | undefined, dataDirectory: string | undefined;
try {
  ...
  const main = path.join(desktopCopy, 'src/main.ts');
  writeFileSync(main, readFileSync(main, 'utf8') + '\nimport "./desktop-probe.ts";\n');
  const parity = [];
  for (const phase of ['save', 'relaunch']) {
    writeFileSync(path.join(desktopCopy, 'src/desktop-probe.ts'), readFileSync(new URL('./desktop-probe.ts', import.meta.url), 'utf8').replace('__PHASE__', phase).replace('__TITLE_VERB__', changedRust ? 'Enter' : 'Use'));
    ...
    const result = await waitForDesktopReport(report, desktop) as { passed: boolean; directory: string };
  ...
  const artifacts: Record<string, { files: ReturnType<typeof inventory>; bytes: number }> = {};
