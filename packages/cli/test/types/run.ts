...
try {
  ...
  assert.deepEqual([...new Set(loaded.typeDiagnostics.map((d: { command: string }) => d.command))].sort(), ['custom', 'wide']);
  assert.ok(loaded.typeDiagnostics.every((d: { line: number; location: string; message: string }) => d.line > 0 && d.location && d.message));
  ...
  const payload = {};
  const bound = loaded.createCommands((...args: unknown[]) => { assert.deepEqual(args, ['nothing', payload, options]); return request; });
  ...
  assert.deepEqual(native, JSON.parse(readFileSync(desktopResult, 'utf8')));
  ...
  for (const platform of ['ios', 'android']) {
    ...
    artifacts[platform] = JSON.parse(readFileSync(path.join(copied, 'manifest.json'), 'utf8'));
  }
  ...
  for (const host of ['react-native', 'lynx']) {
    ...
    writeFileSync(path.join(consumer, 'observed.ts'), `import type { Commands } from './Native Artifacts/commands';\n` + native.map((r: { ok: boolean; value?: unknown; error?: unknown }, i: number) => `export const sample${i} = ${JSON.stringify(r.ok ? r.value : r.error)} satisfies Commands[${JSON.stringify(samples[i]![0])}][${JSON.stringify(r.ok ? 'success' : 'error')}];`).join('\n'));
