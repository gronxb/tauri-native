...
const evidence = path.join(root, 'target/tauri-mobile-composition/lynx-android');
...
function run(label: string, command: string, args: string[], cwd = producer) {
  console.log(`> lynx-android: ${label}`);
...
try {
  ...
  writeFileSync(path.join(renderer, 'package.json'), '{"name":"runtime-lynx-proof","private":true,"type":"module"}\n');
  ...
  for (const file of ['RuntimeProof.java', 'LynxProofModule.java']) cpSync(path.join(sources, 'android', file), path.join(java, file));
  cpSync(path.join(sources, 'android/LynxActivity.kt.fixture'), path.join(java, 'MainActivity.kt'));
  const assets = path.join(native, 'app/src/main/assets'); mkdirSync(assets, { recursive: true });
  cpSync(path.join(renderer, 'dist/main.lynx.bundle'), path.join(assets, 'main.lynx.bundle'));
  ...
  for (const library of libraries) {
    const extracted = spawnSync('unzip', ['-p', apk, library], { maxBuffer: 128 * 1024 * 1024 });
    assert.equal(extracted.status, 0);
    const file = path.join(elfDirectory, path.basename(library)); writeFileSync(file, extracted.stdout);
  ...
  writeFileSync(path.join(evidence, 'report.json'), JSON.stringify({ passed: true, renderer: 'Lynx 4.0.1', platform: 'android',
  ...
  console.log(`PASS: Lynx and Tauri Android coexistence, real renderer interaction, remount and background/resume. ${evidence}/report.json`);
} finally {
