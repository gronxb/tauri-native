...
function receiptTest(run: (directory: string, seal: () => void) => void) {
  ...
  const contents: Record<string, string> = {
    ...
    'include/tauri_native_runtime.h': '// ABI 3', 'android/settings.gradle': '', 'android/tauri.settings.gradle': '',
  ...
  const manifest: RetainedAndroidArtifact = {
    formatVersion: 2, abiVersion: 3, platform: 'android', generator: { name: '@tauri-native/cli', version: '1.0.0-rc.0' },
    ...
    plugins: {}, native: [{ abi: 'arm64-v8a', path: 'android/app/src/main/jniLibs/arm64-v8a/libfixture.so' }], commands: 'commands.json', callers: 'callers.json',
