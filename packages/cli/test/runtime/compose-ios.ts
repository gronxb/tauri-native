...
const evidence = path.join(root, 'target/tauri-mobile-composition/lynx-ios');
...
try {
  ...
  for (const file of ['LynxProof.h', 'LynxProof.m']) cpSync(path.join(sources, 'ios', file), path.join(integration, file));
  ...
  writeFileSync(main, '#import "LynxProof.h"\n' + originalMain.replace('ffi::start_app();', '@autoreleasepool { [LynxProof install]; }\n\tffi::start_app();'));
  ...
  writeFileSync(path.join(native, 'Podfile'), `source 'https://cdn.cocoapods.org/'
platform :ios, '14.0'
use_modular_headers!
...
`);
  ...
  writeFileSync(path.join(evidence, 'report.json'), JSON.stringify({ passed: true, renderer: 'Lynx 4.0.1', platform: 'ios',
  ...
  console.log(`PASS: Lynx and Tauri iOS coexistence, real renderer interaction, remount and background/resume. ${evidence}/report.json`);
} finally {
