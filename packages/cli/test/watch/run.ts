...
const timings: Record<string, number> = {}; let watcher: import('node:child_process').ChildProcess | undefined; let watchLog = ''; let sequence = 0;
...
function exportStep(name: string, extra = ['--incremental'], expected = 0, platform = 'android') {
...
function control(value: Record<string, unknown>) { writeFileSync(path.join(producer, 'build-control.json'), JSON.stringify({ label: 'normal', ...value })); }
...
async function until(predicate: () => boolean, label: string, timeout = 90000) {
...
try {
  ...
  const configFile = path.join(producer, 'src-tauri/tauri.conf.json'); const config = JSON.parse(readFileSync(configFile, 'utf8'));
...
`);
  ...
  const frontendFile = path.join(producer, 'src/main.ts'); const frontend = readFileSync(frontendFile, 'utf8');
  ...
  const concurrentResults = await Promise.allSettled(variants.map((variant, index) => new Promise<void>((resolve, reject) => {
