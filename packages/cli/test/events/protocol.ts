...
import type { BridgeMessage, WebViewBridge } from '../../../../scripts/webview-types.ts';
import type { Event, EventTarget } from '@tauri-apps/api/event';

const root = fileURLToPath(new URL('../../../..', import.meta.url));
const apiDirectory = path.join(root, 'examples/tauri/node_modules/@tauri-apps/api');
assert.equal(JSON.parse(readFileSync(path.join(apiDirectory, 'package.json'), 'utf8')).version, '2.11.1');
...
async function document<T extends 'event.js' | 'path.js' = 'event.js'>(file: string, api: T = 'event.js' as T) {
  ...
  const lifecycle = {} as Record<'pagehide' | 'pageshow' | 'load', () => void>;
  const window = {
    document: { readyState: 'loading' },
    addEventListener(name: 'pagehide' | 'pageshow' | 'load', callback: () => void) { lifecycle[name] = callback; },
    webkit: { messageHandlers: { tauriNative: { postMessage(value: BridgeMessage) { messages.push(value); } } } },
    TauriNativeBridge: { postMessage(value: string) { messages.push(JSON.parse(value)); } },
  } as unknown as { document: { readyState: string } } & WebViewBridge;
  ...
  return { api: module.namespace as T extends 'path.js' ? typeof import('@tauri-apps/api/path') : typeof import('@tauri-apps/api/event'), window, lifecycle, messages, id: () => messages.findLast(m => m.type === 'open')!.document! };
}
...
for (const file of files) {
  ...
  test(`${file}: actual Tauri event API, payloads, once/unlisten and isolated generations`, async () => {
    ...
    const received: Event<unknown>[] = [], other: Event<unknown>[] = [], once: unknown[] = [];
    ...
    await Promise.resolve();
    assert.equal((received.at(-1)!.payload as { from: string }).from, 'host');
    await unlisten();
  ...
  test(`${file}: unsupported scopes fail explicitly and failed listeners release callbacks`, async () => {
    ...
    for (const invalid of ['main', { kind: 'App' }, { kind: 'Window', label: 'main' }, { kind: 'Webview', label: 'other' }]) {
      await assert.rejects(host.api.emitTo(invalid as EventTarget, 'selected', 1), /unsupported_event_target/);
    }
