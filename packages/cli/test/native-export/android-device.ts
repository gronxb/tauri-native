...
export async function verifyAndroidConsumer(preparedFile: string) {
  ...
  let serial!: string, installed = false;
  function host(command: string, args: string[], options: Omit<import('node:child_process').SpawnSyncOptions, 'encoding'> = {}) {
    const result = spawnSync(command, args, { env: hostEnvironment, encoding: 'utf8', ...options });
    assert.equal(result.status, 0, `${command}: ${result.error ?? ''}\n${result.stdout}\n${result.stderr}`);
    return result.stdout;
  }
  function device(args: string[]) { return host(adb, ['-s', serial, ...args]); }
  try {
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  assert(process.argv[2], 'Usage: node android-device.ts CONSUMER_PREPARED_JSON');
