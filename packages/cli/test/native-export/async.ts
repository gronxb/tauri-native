...
try {
  ...
  for (const responses of Object.values(JSON.parse(readFileSync(nativeResult, 'utf8')))) {
    assert.deepEqual(responses, JSON.parse(readFileSync(desktopResult, 'utf8')));
  }
  ...
  writeFileSync(path.join(evidence, 'export-report.json'), JSON.stringify({
    ...
    parity: JSON.parse(readFileSync(nativeResult, 'utf8')), sourceHashes: before,
