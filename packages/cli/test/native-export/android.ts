...
import { verifyAndroidConsumer } from './android-device.ts';
...
const newest = (directory: string) => readdirSync(directory).sort((a, b) => a.localeCompare(b, 'en', { numeric: true })).at(-1)!;
...
function host(command: string, args: string[], options: Omit<import('node:child_process').SpawnSyncOptions, 'encoding'> = {}) { return run(command, args, { env: hostEnvironment, ...options }); }
...
try {
  ...
  const configBytes = readFileSync(configPath); const broken = JSON.parse(configBytes.toString('utf8'));
  ...
  for (const [damage, expected] of [['alignment', /not 16 KB aligned/], ['api', /Expected Android API 24/], ['soname', /Incorrect Android library SONAME/], ['architecture', /Incorrect Android ELF architecture/], ['dependency', /Unbundled or unavailable Android dependency/]] as const) {
