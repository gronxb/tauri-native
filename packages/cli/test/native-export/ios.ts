...
function host(args: string[], options: Omit<import('node:child_process').SpawnSyncOptions, 'encoding'> = {}) { return run('xcrun', args, { env: hostEnvironment, ...options }); }
...
try {
  ...
  const brokenConfig = JSON.parse(configBytes.toString('utf8')); brokenConfig.build.beforeBuildCommand = 'node -e "process.exit(23)"';
  ...
  for (const damage of ['architecture', 'header']) {
    assert.throws(() => publishArtifacts(output, stage => {
      ...
      const library = path.join(stage, manifest.native.find((slice: { variant: string; path: string }) => slice.variant === 'simulator').path);
  ...
  const manifest = JSON.parse(readFileSync(path.join(relocated, 'manifest.json'), 'utf8'));
  const library = path.join(relocated, manifest.native.find((slice: { variant: string; path: string }) => slice.variant === 'simulator').path);
  ...
  const runtimes = JSON.parse(host(['simctl', 'list', 'runtimes', '--json'])).runtimes as { isAvailable: boolean; version: string; identifier: string; supportedDeviceTypes: { productFamily: string; identifier: string }[] }[];
  const devices = JSON.parse(host(['simctl', 'list', 'devices', '--json'])).devices as Record<string, { udid: string; isAvailable: boolean; state: string }[]>;
