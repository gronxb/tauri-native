...
M7–M8 implementation is in progress: #42 has real desktop and all four RN/Lynx mobile native dispatch evidence with explicit caller grants and original Tauri capability checks. The [verified support matrix](https://github.com/gronxb/tauri-native/blob/main/docs/retained-support.md) maps standalone Tauri and packed RN/Lynx native plugin/permission execution, source-free consumers, retained views and pending-callback retirement to their evidence. Expo native modules and actual prebuild/config-plugin generation also have Release execution. The first two #43 and #47 acceptance items are checked against those records; format 2 migration is documented for #44. Target-specific dependency selection now preserves inactive desktop dependencies and configured optional plugins, with a desktop baseline and 24 fresh source-free mobile UI flows ([evidence](https://github.com/gronxb/tauri-native/blob/main/docs/evidence/retained-target-dependencies-2026-09-11.json)). Broader lifecycle, retained CI, physical devices and independent adoption remain open. No M7–M8 issue is closed from these scoped checks.

The [Android Activity recreation comparison](https://github.com/gronxb/tauri-native/blob/main/docs/evidence/retained-activity-recreation-2026-09-11.json) adds fifteen native UI flows across ordinary Tauri and packed RN/Lynx, after location permission grant. Two recreations per app preserve process/state/setup and notes; composed listeners retire and fresh events arrive once. Fresh/pending permissions, Expo recreation and broader navigation/rotation remain open. The evidence preserves a log-collection failure and unresolved standalone initial permission-response and post-recreation link UI failures; these scoped results do not close a release gate.

The [fresh permission correction](https://github.com/gronxb/tauri-native/blob/main/docs/evidence/retained-recreation-permissions-2026-09-11.json) addresses the first request after Activity recreation. Original Tauri and both SDKs reproduce the stale launcher error; export now re-registers launchers in the copied Tauri dependency while preserving producer source and original runtime/plugin initialization. A fresh source-free Release export passes twelve native UI flows; packed RN/Lynx add fourteen flows covering OS denial, another recreation, grant, save and deep link. Pending callbacks across recreation, Expo recreation, broader navigation/rotation, retained CI and device/adopter gates remain open. The standalone dependency and earlier unresolved failures remain unchanged.

The [pending permission comparison](https://github.com/gronxb/tauri-native/blob/main/docs/evidence/retained-pending-recreation-2026-09-11.json) adds eighteen packed RN/Lynx Android UI flows and four real Activity recreations while OS dialogs remain pending. Original Tauri denial/grant callbacks complete once on the replacement Activities, old continuations save no sentinel note, and state/setup, later save and deep link remain intact. The previous RN artifact fails to complete the original callback after OS denial. These test-only checks reuse the corrected artifact; RN-owned permissions, Expo recreation, broader navigation/channel behavior and retained CI/device/adopter acceptance remain open.
