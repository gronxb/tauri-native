...
M8 #46 is now IN PROGRESS with a package-owned Android Lynx host, NativeModule and retained session/event API. The actual npm tarball passes a source-free, non-debuggable arm64 Release/R8 execution beside the unchanged Tauri frontend: shared state/setup, separate caller/Tauri/OS permissions, location save, background deep links, native listener cleanup at renderer replacement and fresh event delivery. All eleven native libraries and APK 16 KB alignment pass. [Lynx Android retained SDK evidence](https://github.com/gronxb/tauri-native/blob/main/docs/evidence/retained-lynx-android-2026-09-09.json). Automatic composition/autolinking, retained TauriView, iOS package integration, RN/Expo parity, pending-OS-callback renderer retirement and the complete six-combination/device/adopter gates remain open. No issue or release is closed by this Android increment.
