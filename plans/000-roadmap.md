...
M7 implementation is in progress: #42 has real desktop and all four RN/Lynx mobile native dispatch evidence with explicit caller grants and original Tauri capability checks. Package-owned C/JNI sessions preserve the original Tauri bootstrap. #43 has an ordinary official-plugin location-note fixture with completed desktop/iOS/Android execution: real Swift/Kotlin position callbacks, Tauri ACL, OS permission denial/grant, deep-link resume and process-relaunch persistence pass. Composed plugin callers and callback retirement remain open. No M7–M8 issue is closed from compilation or a partial native run.
