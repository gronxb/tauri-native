...
The first actual renderer-package path now passes: the packed Lynx Android retained SDK invokes the original Kotlin geolocation plugin, preserves separate Tauri/native/OS permission decisions and observes original save/deep-link events through background/resume and renderer replacement. This is a non-debuggable Release/R8 APK; actual native listeners drop to zero before replacement and return to one. [Lynx Android retained SDK evidence](https://github.com/gronxb/tauri-native/blob/main/docs/evidence/retained-lynx-android-2026-09-09.json). iOS/RN/Expo package paths and renderer destruction during a pending OS permission callback remain open.
