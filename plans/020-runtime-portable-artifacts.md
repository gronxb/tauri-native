...
A packed Lynx Android SDK now consumes the existing format 2 arm64 Release artifact in a relocated path with no Rust build/toolchain access. Non-debuggable Release/R8 execution passes original frontend and package NativeModule state/permission/plugin/event/remount checks; the input artifact receipt remains unchanged. APK ABIs are restricted to exported Tauri slices, and all eleven packaged ELF libraries plus APK 16 KB alignment pass. [Lynx Android retained SDK evidence](https://github.com/gronxb/tauri-native/blob/main/docs/evidence/retained-lynx-android-2026-09-09.json). This is an SDK consumption run of an existing artifact, not a fresh export or another proof of the Android origin-capture fix. Automatic composition, retained-view integration, iOS/RN/Expo package parity and complete release acceptance remain open.
